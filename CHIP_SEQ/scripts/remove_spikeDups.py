...
# Function to get a list (set) with all the read IDs from a bamfile. First it will index the bam.
...
# This function will open the bam files and write to a new ones just those reads that aren't shared by both bam files
	...
	# Open the new bam files where we are going to write the output
